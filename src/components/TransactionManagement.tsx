...
export default function TransactionManagement() {
  ...
  const [transactions, setTransactions] = useState([
    { id: '1', bookCode: 'F001', bookTitle: 'Harry Potter', memberName: 'John Doe', loanDate: '2023-05-01', dueDate: '2023-05-15', returnDate: '', status: 'Active' },
    { id: '2', bookCode: 'NF001', bookTitle: 'Sapiens', memberName: 'Jane Smith', loanDate: '2023-05-05', dueDate: '2023-05-19', returnDate: '2023-05-18', status: 'Returned' },
  ]);
  ...
  const handleReturnSubmit = (e: React.FormEvent) => {
    ...
    const updatedTransactions = transactions.map(t => 
      t.id === returnForm.transactionId ? { ...t, returnDate: new Date().toISOString().split('T')[0], status: 'Returned' } : t
    );
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">🔄 Manajemen Transaksi</h2>
      
      <div className="flex border-b mb-6">
        <button
          className={`px-4 py-2 font-medium ${activeTransactionTab === 'borrow' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
          onClick={() => setActiveTransactionTab('borrow')}
        >
          <FiArrowDown className="inline mr-2" /> Peminjaman
        </button>
        <button
          className={`px-4 py-2 font-medium ${activeTransactionTab === 'return' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
          onClick={() => setActiveTransactionTab('return')}
        >
          <FiArrowUp className="inline mr-2" /> Pengembalian
        </button>
        <button
          className={`px-4 py-2 font-medium ${activeTransactionTab === 'history' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
      ...
      {activeTransactionTab === 'borrow' && (
        <div>
          <h3 className="font-medium mb-4">Form Peminjaman Buku</h3>
          <form onSubmit={handleBorrowSubmit} className="bg-gray-50 p-4 rounded-lg mb-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setBorrowForm({...borrowForm, bookCode: e.target.value})}
                  required
                >
                  <option value="">Pilih Buku</option>
                  {books.filter(b => b.status === 'Available').map(book => (
                    <option key={book.code} value={book.code}>{book.title} ({book.code})</option>
                  ))}
                </select>
              </div>
              <div>
      ...
      {activeTransactionTab === 'return' && (
        <div>
          <h3 className="font-medium mb-4">Form Pengembalian Buku</h3>
          <form onSubmit={handleReturnSubmit} className="bg-gray-50 p-4 rounded-lg mb-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                >
                  ...
                  {transactions.filter(t => t.status === 'Active').map(transaction => (
                    <option key={transaction.id} value={transaction.id}>
                      {transaction.bookTitle} - {transaction.memberName} (Jatuh tempo: {transaction.dueDate})
                    </option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Denda (jika ada)</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setReturnForm({...returnForm, fine: parseInt(e.target.value) || 0})}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white border rounded-lg overflow-hidden">
            ...
            <tbody>
              {transactions
                .filter(t => 
                  (activeTransactionTab === 'borrow' && t.status === 'Active') ||
                  (activeTransactionTab === 'return' && t.status === 'Active') ||
                  (activeTransactionTab === 'history')
                )
                .map((transaction, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="py-2 px-4 border-b">{transaction.id}</td>
                    <td className="py-2 px-4 border-b">{transaction.bookTitle} ({transaction.bookCode})</td>
                    <td className="py-2 px-4 border-b">{transaction.memberName}</td>
                    <td className="py-2 px-4 border-b">{transaction.loanDate}</td>
                    <td className="py-2 px-4 border-b">{transaction.dueDate}</td>
                    <td className="py-2 px-4 border-b">{transaction.returnDate || '-'}</td>
                    <td className="py-2 px-4 border-b">
                      <span className={`px-2 py-1 rounded-full text-xs ${
                        transaction.status === 'Active' ? 'bg-blue-100 text-blue-800' : 
                        'bg-green-100 text-green-800'
                      }`}>
                        {transaction.status === 'Active' ? 'Aktif' : 'Dikembalikan'}
                      </span>
                    </td>
                  </tr>
                ))}
